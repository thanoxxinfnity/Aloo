...
export const DEFAULT_SETTINGS = {
  ...
  /**
   * PLACEMENT OF THE ENVIRONMENT, and why these numbers.
   *
   * The bundled environment is a galaxy point cloud. Sat close and centred it
   * wraps the character like confetti and washes the frame out — nothing that
   * size is ever that near. Pushed to ~600 units it becomes one distant object
   * with sky around it, and the sideways offset moves its bright core off the
   * character's head, where it would otherwise read as a halo she is wearing
   * rather than as a place she is standing in.
   */
  spaceFitRadius: 95,
  spaceOffsetX: -70,
  spaceOffsetY: 70,
  ...
  bloomIntensity: 0.9,
  // Backdrop: 'model' (the supplied environment GLB, shown on its own) or
...
function migrate(saved) {
  ...
  /* The generated galaxy backdrop was removed in favour of showing the supplied
   * environment model on its own. A stored 'galaxy' would otherwise resolve to
   * nothing and leave the sky empty. */
  if (out.backdrop === 'galaxy') out.backdrop = 'model';
