...
export const DEFAULT_SETTINGS = {
  ...
  // --- TV control -----------------------------------------------------------
  /**
   * LG webOS, over the local network. Bluetooth is not a control channel for a
   * television — see services/lgWebosService.js.
   *
   * `tvClientKey` is issued by the TV the first time the user accepts the
   * on-screen prompt. Storing it is what stops that prompt appearing on every
   * single connection afterwards.
   */
  tvEnabled: false,
  tvHost: '',
  tvClientKey: '',
  /**
   * Which port answered last time. Older sets listen on plain 3000; webOS 6 and
   * later closed it and moved to 3001 behind a self-signed certificate. Both
   * are tried on a first connection — this just records the winner so later
   * launches do not pay for the discovery again.
   */
  tvSecurePort: false,
  /**
   * The TV's MAC address and model name, both read off the TV itself the first
   * time ALOO connects.
   *
   * The MAC is what makes "TV on karo" possible: a set that is off runs no
   * server, so the only thing that reaches it is a Wake-on-LAN packet addressed
   * to its network card. It can only be collected while the TV is awake, which
   * is why it is captured on connect rather than asked for.
   */
  tvMac: '',
  tvName: '',
  // Recognise "TV band karo" locally and act, instead of sending it to the
  // model and waiting a second for a round trip to say the same thing.
  tvVoiceCommands: true,
  /**
   * "Hotspot on karo", "wifi settings kholo" — opens the phone's own settings
   * page. It cannot flip the switch: Android reserves tethering control for
   * system apps. See services/deviceSettingsService.js.
   */
  deviceCommands: true,
  ...
  // --- HUD ------------------------------------------------------------------
  /**
   * 'auto' reads the device (see lib/quality.js) and picks a cost profile;
   * the named tiers override it. This is the single control for whether the
   * scene runs on a 4GB phone.
   */
  quality: 'auto',
  // Both of these draw OVER the character. They are atmosphere, and atmosphere
  // that hides the thing you came to look at is a bad trade, so they default
  // off and are there for anyone who wants the CRT look back.
  hudGrid: false,
  hudScanlines: false,
  showVisualizer: true,
  showTelemetry: true,
...
function migrate(saved) {
  ...
  /* Scanlines and the grid used to default on, and they sit ON TOP of the
   * character — the most common complaint about her being hard to see. Anyone
   * still carrying the old default gets the clearer view; the toggles remain. */
  if (out.hudScanlines === true && out.__hudChosen !== true) out.hudScanlines = false;
  if (out.hudGrid === true && out.__hudChosen !== true) out.hudGrid = false;
