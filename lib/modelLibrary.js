...
export const BUILT_IN_MODELS = [
  {
    id: 'builtin:miku',
    kind: 'avatar',
    label: 'Miku (bundled, default)',
    url: '/models/miku.glb',
    builtin: true,
    // Stated plainly because it decides what the character can do: an MMD/PMX
    // export whose converter dropped the morphs, so there is nothing to drive a
    // mouth with — no visemes AND no jaw bone. Everything else is rich.
    note: '471 bones (Japanese/MMD) · working eye bones · NO mouth: no visemes, no jaw',
  },
  {
    id: 'builtin:avatar',
    kind: 'avatar',
    ...
    builtin: true,
    note: '59 bones · jaw bone, so lip-sync works · inert eye bones · no visemes',
  },
  {
    id: 'builtin:holo',
    kind: 'avatar',
