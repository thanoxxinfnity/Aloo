...
export const EMOTIONS = {
  neutral: {
    ...
    face: { smile: 0.12 },
    gestures: {
      nod: 2, tilt: 2.5, openPalms: 1.2, sway: 2, clasp: 1,
      curiousTilt: 2.5, rock: 2, stretch: 1.5, fidget: 2, perkUp: 1.2, peek: 1.2,
    },
  },
  warm: {
    label: 'Warm',
    ...
    gestures: {
      nod: 3, tilt: 3, handToChest: 2, openPalms: 2, hairTouch: 1.2, sway: 1.5,
      curiousTilt: 2.5, peek: 1.8, fidget: 1.2, rock: 1.5,
    },
  },
  happy: {
    label: 'Happy',
    ...
    gestures: {
      nod: 3, bounce: 2.5, openPalms: 2, handRaise: 1.5, tilt: 2,
      perkUp: 2, curiousTilt: 1.5, rock: 1.2,
    },
  },
  excited: {
    label: 'Excited',
    ...
    gestures: {
      bounce: 3, handRaise: 3, openPalms: 2.5, nod: 2, pointForward: 1.5, perkUp: 2.2,
    },
  },
  thoughtful: {
    label: 'Thoughtful',
    ...
    gestures: {
      tilt: 3, handToChin: 2.5, slowNod: 2, hairTouch: 1.5, clasp: 2,
      curiousTilt: 2, stretch: 1.2,
    },
  },
  curious: {
    label: 'Curious',
    ...
    gestures: {
      tilt: 4, leanIn: 2.5, openPalms: 1.5, nod: 1.5,
      curiousTilt: 3.5, peek: 2.5, perkUp: 1.5,
    },
  },
  concerned: {
    label: 'Concerned',
  ...
  playful: {
    ...
    gestures: {
      tilt: 3.5, bounce: 2, hairTouch: 2, shrug: 2, handRaise: 1.5,
      curiousTilt: 2.5, peek: 2, rock: 1.5, fidget: 1.5,
    },
  },
};

/**
 * Moods the character drifts between when nobody is talking to her.
 *
 * Deliberately only the calm ones. Drifting into 'excited' or 'apologetic'
 * unprompted reads as a glitch — those are REACTIONS, and a reaction with
 * nothing to react to is worse than stillness.
 */
export const IDLE_MOODS = ['neutral', 'warm', 'curious', 'thoughtful', 'playful'];
...
const GESTURES = {
   ...
   * IDLE LIFE
   *
   * These exist because the bundled rig has NO facial blendshapes and its eye
   * bones carry zero skin weight — verified against the GLB: 0 morph targets,
   * and 0 of 20,294 vertices weighted to LeftEye/RightEye. So "expression" on
   * this model cannot come from the face at all. Every readable beat has to be
   * carried by head, neck, spine and arms, which is what these do. On a model
   * that DOES ship blendshapes the face layer lights up on top of them
   * automatically — nothing here needs changing.
   * ------------------------------------------------------------------------ */

  /** The head-tilt-and-hold. The single strongest "she is listening" cue. */
  curiousTilt: () => {
    const side = pick();
    const amp = rand(0.16, 0.26);
    return {
      // Long, with a long hold: the charm is in how long it stays there.
      dur: rand(2.6, 4.2),
      apply: (p, u, k) => {
        const e = envelope(u, 0.75);
        p.head.z += side * amp * e * k;
        p.head.y += side * 0.06 * e * k;
        p.head.x -= 0.03 * e * k; // chin up a touch — attentive, not sleepy
        p.neck.z += side * 0.04 * e * k;
      },
    };
  },

  /** A small "oh!" — she notices something and straightens up. */
  perkUp: () => {
    const amp = rand(0.05, 0.09);
    return {
      dur: rand(0.9, 1.5),
      apply: (p, u, k) => {
        // Fast rise, slow settle: a reaction, not a stretch.
        const e = u < 0.22 ? u / 0.22 : Math.pow(1 - (u - 0.22) / 0.78, 1.7);
        p.head.x -= amp * e * k;
        p.spine.x -= amp * 0.45 * e * k;
        p.leftShoulder.z -= 0.05 * e * k;
        p.rightShoulder.z += 0.05 * e * k;
      },
    };
  },

  /** Shoulder roll — the thing people do when they have been still too long. */
  stretch: () => {
    const side = pick();
    return {
      dur: rand(2.4, 3.6),
      apply: (p, u, k) => {
        const e = envelope(u, 0.5);
        const roll = Math.sin(u * TAU) * e;
        p.leftShoulder.z -= roll * 0.08 * k;
        p.rightShoulder.z += roll * 0.08 * k;
        p.spine.z += side * roll * 0.03 * k;
        p.head.z -= side * roll * 0.05 * k;
      },
    };
  },

  /** Idle hand fidget — small, low, entirely in the forearm. */
  fidget: () => {
    const side = pick();
    const cycles = Math.round(rand(2, 4));
    ...
    return {
      ...
      apply: (p, u, k) => {
        ...
        const fore = side < 0 ? p.leftForeArm : p.rightForeArm;
        fore.y += -side * amp * e * k;
        fore.z += Math.sin(u * TAU * cycles) * 0.1 * e * k;
      },
    };
  },

  /** Weight shift onto one hip, with the counter-tilt that makes it read. */
  rock: () => {
    const side = pick();
    return {
      dur: rand(3.0, 4.8),
      apply: (p, u, k) => {
        const e = envelope(u, 0.7);
        const s = Math.sin(u * TAU) * e;
        p.hips.z += side * s * 0.06 * k;
        // The shoulders counter-rotate — standing on one leg, not leaning over.
        p.spine.z -= side * s * 0.035 * k;
        p.head.z += side * s * 0.03 * k;
      },
    };
  },

  /** Lean toward the camera, as if peering at you. */
  peek: () => {
    const side = pick();
    return {
      dur: rand(2.0, 3.2),
      apply: (p, u, k) => {
        ...
        p.spine.x -= 0.045 * e * k;
        p.head.x -= 0.05 * e * k;
        p.head.z += side * 0.1 * e * k;
        p.head.y += side * 0.08 * e * k;
      },
    };
  },

  /**
   * "The key goes THERE." Points up at the settings gear.
   *
   * The gear lives at the TOP-RIGHT of the screen. The camera looks down -Z, so
   * screen-right is world +X, which is the avatar's own LEFT side — hence the
   * left arm, hard-coded rather than randomised: a point that lands on the
   * wrong side of the screen is worse than no point at all.
   *
   * The lift is almost entirely elbow. The upper arm is clamped to 0.3rad
   * because this rig rests in an A-pose, where rotating the shoulder outward
   * drives it back toward a T-pose and the arm ends up horizontal like a
   * scarecrow's.
   */
  pointAtSettings: () => {
    /* WHICH ARM IS ON THE RIGHT OF THE SCREEN?
       Not a question the bone NAMES can answer. "LeftHand" is a convention, and
       which convention a rig follows — the character's own left, or the
       viewer's — varies by authoring tool; guessing it wrong sends her pointing
       off the opposite edge of the screen, away from the gear. So AvatarCanvas
       measures the hands' world X once the rig loads and reports which side
       renders on the right (see director.screenRight). */
    const key = director.screenRight === 'right' ? 'right' : 'left';
    const arm = `${key}Arm`;
    const fore = `${key}ForeArm`;
    // The forearm lift is signed the same way the other arm gestures sign it.
    const lift = key === 'left' ? 1 : -1;
    return {
      // Slow, and held long: this is instruction, not a flourish.
      dur: 3.6,
      apply: (p, u, k) => {
        const e = envelope(u, 0.8);
        // Shoulder flexes FORWARD only. Abduction is deliberately absent: on
        // this A-posed rig, swinging the upper arm outward drives it back
        // toward the bind pose, and both arms out level is exactly the T-pose
        // the character is supposed to never show.
        p[arm].x -= 0.26 * e * k;
        p[fore].y += lift * 1.0 * e * k; // the elbow does the raising
        p[fore].z += lift * 0.2 * e * k;
        // She looks where she points — a point nobody follows is just a pose.
        p.head.y += (key === 'right' ? 0.26 : -0.26) * e * k;
        p.head.x -= 0.12 * e * k;
        p.neck.y += (key === 'right' ? 0.08 : -0.08) * e * k;
      },
    };
  },

  /**
   * WAKE-UP — played once, the first time she appears.
   *
   * Without it the app opens on a character standing perfectly still in her
   * rest pose, which reads as a frozen render rather than someone who just came
   * online. Worse, the rest pose of a clip-less humanoid rig is a corrected
   * A-pose that still looks posed rather than alive, so the opening seconds are
   * exactly when the model looks least convincing.
   *
   * Structured as one phrase in three beats rather than three gestures, so the
   * timing cannot be broken up by the scheduler firing something else midway:
   *   0.00-0.35  head down, shoulders in — dormant
   *   0.35-0.62  rises, chest opens, chin lifts — coming online
   *   0.62-1.00  finds the viewer, small greeting lift, settles
   */
  wakeUp: () => {
    const side = pick();
    return {
      ...
      apply: (p, u, k) => {
        if (u < 0.35) {
          // Dormant: curled slightly in on herself, eased OUT of over the beat.
          const d = 1 - u / 0.35;
          p.head.x += 0.16 * d * k;
          p.neck.x += 0.07 * d * k;
          p.spine.x += 0.09 * d * k;
          p.leftShoulder.z += 0.06 * d * k;
          p.rightShoulder.z -= 0.06 * d * k;
        } else if (u < 0.62) {
          // Waking: a breath in, chin up, shoulders opening back.
          const a = (u - 0.35) / 0.27;
          const s = Math.sin(a * Math.PI);
          p.spine.x -= 0.05 * s * k;
          p.head.x -= 0.07 * s * k;
          p.leftShoulder.z -= 0.05 * s * k;
          p.rightShoulder.z += 0.05 * s * k;
          p.head.z += side * 0.05 * s * k;
        } else {
          // Online: turns to the viewer and lifts a hand in greeting.
          const a = (u - 0.62) / 0.38;
          const s = Math.sin(a * Math.PI);
          const arm = side < 0 ? p.leftArm : p.rightArm;
          const fore = side < 0 ? p.leftForeArm : p.rightForeArm;
          arm.x -= 0.18 * s * k;
          fore.y += -side * 0.9 * s * k;
          fore.z += Math.sin(a * TAU * 2) * 0.16 * s * k; // the wave itself
          p.head.z -= side * 0.07 * s * k;
          p.head.x -= 0.03 * s * k;
        }
      },
    };
...
class AnimationDirector {
  constructor() {
    ...
    // Idle mood drift — see update(). Kept switchable so a user who wants a
    // perfectly steady character can turn it off.
    this.idleDrift = true;
    this.driftIn = 12;
    /**
     * Which of the rig's arms renders on the RIGHT of the screen, 'left' or
     * 'right' by bone-name convention. Measured from the loaded rig rather than
     * assumed from names — see AvatarCanvas, and pointAtSettings for why it
     * matters. 'left' is the common case (a character facing the camera shows
     * her left arm on the viewer's right).
     */
    this.screenRight = 'left';
  ...
  update(dt, ctx = {}) {
    ...
    /* ---- Idle mood drift ---------------------------------------------------
       Posture is per-emotion, so an idle character parked on one emotion holds
       exactly one posture forever and every gesture is drawn from the same bag.
       Drifting slowly between the calm emotions changes both — how she stands
       and what she is likely to do next — without any conversation to drive it.
       Only while genuinely idle: a mood set from a real reply must not be
       overwritten mid-answer. */
    if (this.state === 'idle' && this.idleDrift) {
      this.driftIn -= dt;
      if (this.driftIn <= 0) {
        const next = IDLE_MOODS[Math.floor(Math.random() * IDLE_MOODS.length)];
        // A gentle intensity too — idle drift should never look like a reaction.
        this.setEmotion(next, rand(0.45, 0.72));
        this.state = 'idle'; // setEmotion does not change state; keep it explicit
        this.driftIn = rand(18, 38);
      }
    }
    ...
    if (this.nextGestureIn <= 0) {
      ...
      /* IDLE CADENCE.
         This was 8.5-15.5s, which is a very long time to watch someone hold
         still — long enough that the character reads as a paused video rather
         than as someone waiting for you. At 4-8s there is almost always
         something in flight or just settling, and because every gesture is
         generated fresh it still never looks looped. */
      const base = speaking ? 2.1 : 4.2;
      const spread = speaking ? 1.6 : 4;
    ...
    /* ---- The T-pose can never be reached ----------------------------------
       The symmetric clamp above bounds how FAR an upper arm can rotate but not
       in which DIRECTION, and the two directions are not equivalent here. The
       rest pose is a corrected A-pose reached by rotating each upper arm down
       from the bind pose; rotating back the other way walks the arm toward the
       bind pose, and a bind-pose humanoid is a T-pose. Two overlapping gestures
       that each abduct a little therefore sum into arms held straight out —
       which is precisely the "it opens in a T-pose" the character must never
       show, and it is indistinguishable from a bug because it looks like one.

       So abduction gets an eighth of the budget that adduction does. Gestures
       stay free to bring the arms in and across the body, where nothing can
       look broken, and no combination of them can straighten the arms out.

       Sign convention matches the A-pose correction in AvatarCanvas: the left
       arm travels DOWN on negative Z, the right arm on positive Z. */
    const ABDUCT = LIMITS.leftArm * 0.125;
    if (p.leftArm.z > ABDUCT) p.leftArm.z = ABDUCT;
    if (p.rightArm.z < -ABDUCT) p.rightArm.z = -ABDUCT;
