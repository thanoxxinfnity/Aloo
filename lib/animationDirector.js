...
class AnimationDirector {
  ...
  update(dt, ctx = {}) {
    ...
    /* THE T-POSE GUARD LIVES WITH THE RIG, NOT HERE.
       This used to clamp `leftArm.z > 0` and `rightArm.z < 0`, which encodes
       one vendor's bind orientation: it assumes the left arm travels DOWN on
       negative Z. On a rig where that sign is reversed — an MMD export, for one
       — the clamp throttles the downward direction to a couple of degrees and
       leaves the upward one wide open, which is exactly backwards: the
       character barely moves AND can still drift toward a T-pose.

       The director cannot know a rig's convention; AvatarCanvas measures it. So
       the guard is applied there, against the arm's real angle from vertical.
       See `armDownSign` in AvatarCanvas. */
