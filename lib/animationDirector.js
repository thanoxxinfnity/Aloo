...
const GESTURES = {
  ...
  shrug: () => {
    ...
    return {
      ...
      apply: (p, u, k) => {
        ...
        p.leftArm.z -= arm * e * k;
        p.rightArm.z += arm * e * k;
        p.leftForeArm.y += arm * 0.8 * e * k;
        p.rightForeArm.y -= arm * 0.8 * e * k;
        p.head.x += 0.03 * e * k;
      },
    };
  },
  ...
  openPalms: () => {
    const amp = rand(0.16, 0.3);
    const asym = rand(0.7, 1.0);
    return {
      dur: rand(1.2, 2.2),
      apply: (p, u, k) => {
        const e = envelope(u, 0.55);
        p.leftArm.z -= amp * e * k;
        p.rightArm.z += amp * asym * e * k;
        p.leftForeArm.y += amp * 1.4 * e * k;
        p.rightForeArm.y -= amp * 1.4 * asym * e * k;
      },
    };
  },
  ...
  handRaise: () => {
    ...
    return {
      ...
      apply: (p, u, k) => {
        ...
        arm.z += -side * amp * e * k;
        arm.x -= 0.2 * e * k;
        fore.y += -side * rand(0.2, 0.35) * e * k;
        fore.z += Math.sin(u * TAU * wob) * 0.12 * e * k;
      },
    };
  },
  ...
  clasp: () => {
    ...
    return {
      ...
      apply: (p, u, k) => {
        ...
        p.leftArm.z -= amp * 0.4 * e * k;
        p.rightArm.z += amp * 0.4 * e * k;
        p.leftForeArm.y += amp * 3.2 * e * k;
        p.rightForeArm.y -= amp * 3.2 * e * k;
      },
    };
  },
...
class AnimationDirector {
  constructor() {
    ...
    this.enabled = true;
    this.scale = 1; // user-facing intensity multiplier
